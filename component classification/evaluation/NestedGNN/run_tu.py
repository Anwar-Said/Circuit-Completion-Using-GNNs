...
parser.add_argument('--data', type=str, default='ltspice_examples')
...
args.res_dir = os.path.join(file_dir, 'results/Circuit{}'.format(args.save_appendix))
...
for dataset_name, Net in product(datasets, nets):
    ...
    for num_layers, hidden, h in combinations:
        ...
        dataset = MyOwnDataset("data/",
            ...
            args.use_rd, 
            args.max_nodes_per_hop)
        print("dataset loaded",dataset.num_features,dataset.num_classes)
        
        
        model = Net(dataset, num_layers, hidden, args.node_label!='no', args.use_rd)
        loss, acc, std, f1,f1_std = cross_val_method(
            dataset,
            model,
            folds=3,
            epochs=args.epochs,
        ...
        if loss < best_result[0]:
            best_result = (loss, acc, std,f1,f1_std)
            best_hyper = (num_layers, hidden, h)

    desc = 'auc:{:.3f} ± {:.3f},f1:{:.3f} ± {:.3f}'.format(
